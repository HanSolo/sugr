    ...
    var body: some View {
        GeometryReader { geometry in
            Canvas(opaque: false, colorMode: .linear, rendersAsynchronously: false) { ctx, size in
                ...
                let scaleXFontSize : Double = minSize * 0.03
                ...
                if !self.model.last13Entries.isEmpty {
                    ...
                    for n in 0..<13 {
                        ...
                        topTick.move(to: CGPoint(x: x, y: chartOffset + scaleXFontSize * 1.2))
                        topTick.addLine(to: CGPoint(x: x, y: topY))
                        ctx.stroke(topTick, with: Constants.GC_WHITE, lineWidth: 1)
                        
                        var vLine : Path = Path()
                        vLine.move(to: CGPoint(x: x, y: chartOffset + scaleXFontSize))
                        ...
                        bottomTick.addLine(to: CGPoint(x: x, y: chartOffset + chartHeight - scaleXFontSize * 1.2))
                        ctx.stroke(bottomTick, with: Constants.GC_WHITE, lineWidth: 1)
                    ...
                    for entry in self.model.last13Entries {
                        ...
                        if x > chartOffset {
                            ...
                            let valueText : Text = Text(verbatim: String(format: unitMgDl ? "%.0f" : "%.1f", unitMgDl ? entry.sgv : Helper.mgToMmol(mgPerDl: entry.sgv))).foregroundStyle(fgdColor).font(scaleXFont)
                            ctx.draw(valueText, at: CGPoint(x: x + dotRadius, y: entry.sgv > 350 ? y + scaleXFontSize + dotRadius * 2 : y - scaleXFontSize))
                        }
                    }
                }
            }
